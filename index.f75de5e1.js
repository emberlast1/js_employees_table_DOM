...
const sortBy = (columnIndex)=>{
    ...
    rows.sort((rowA, rowB)=>{
        ...
        if (currentColumn !== columnIndex) sortOrder = "asc";
        switch(columnIndex){
            case 0:
                currentColumn = 0;
            ...
            case 4:
                currentColumn = 4;
                return sortOrder === "asc" ? +cellA.slice(1).split(",").join("") - +cellB.slice(1).split(",").join("") : +cellB.slice(1).split(",").join("") - +cellA.slice(1).split(",").join("");
            default:
                return 0;
        }
    });
...
function validation(eName, age) {
    ...
    if (eName.length < 4) {
        nameError = true;
        pushNotification("Name error", "The name must be 4 or more characters long", "error");
...
function saveChanges(cell, input) {
    ...
    if (newValue.length > 0) cell.textContent = newValue;
    else cell.textContent = input.defaultValue;
}
function editCell() {
    const tBody = document.querySelector("tbody");
    ...
    cells.forEach((cell)=>{
        cell.addEventListener("dblclick", (e)=>{
            ...
            cellInput.addEventListener("keypress", (ev)=>{
                if (ev.key === "Enter") saveChanges(cell, cellInput);
            });
        });
    });
}
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    form.addEventListener("submit", (e)=>{
        ...
        if (validationCheck) {
            addNewEmployee(nameInput, positionInput, officeInput, ageInput, normSalary);
            form.reset();
        }
    });
    editCell();
